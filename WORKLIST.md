...
| Merieke Ri Berit — misparse backlog root-cause #6 flagged "collapsed or disjunction," but that part (leaves-battlefield-or-becomes-untapped delayed trigger) is already correctly parsed via `WhenNextEvent{trigger, or_trigger}`; real remaining bug is "It can't be regenerated" failing to bind to the nested Destroy effect inside the delayed trigger (spuriously creates a bogus separate `CantBeRegenerated` static grant with an arbitrary `UntilEndOfTurn` duration instead of setting `cant_regenerate: true` on the Destroy, unlike Terror/Nekrataal's correct simple-case handling) | old-school-1993-95 | in-progress | rykerwilliams-old-school | 2026-07-12 | `fix/merieke-ri-berit-cant-regenerate` | — |
