...
**Current holder:** `void-mirror-603-4 since 2026-09-15T19:54Z (expected ~40m, PR #8896 review follow-up test + revert-proof; pid 3102016)`
...
|---|---|---|---|---|---|---|
| **[Mass mutual control exchange, issue #4731]** Reins of Power / Twist Allegiance — BLOCKED on a design decision, not implemented. The parser has no "each gain control of … the other controls" handling, so the swap still collapses to `GainControlAll`, which always gives control to the caster. `/add-engine-variant` verdict: **REFUSE_WITH_REFACTOR**. The control family already splits along two axes as separate variants (`GainControl`/`GiveControl` differ only by recipient; `GainControl`/`GainControlAll` only by single vs. all), so a `GiveControlAll` or `ExchangeControlAll` would proliferate; the recipient axis should be parameterized first (within CR 613.1b). Even then, a mutual swap needs both populations captured once at resolution (CR 611.2c), not two sequential control changes. Awaiting a decision on whether to open that refactor round. Uncommitted probe test on branch fix/reins-of-power-4731. | old-school-1993-95 | blocked | void-mirror-603-4 | 2026-09-15 | fix/reins-of-power-4731 | — |
