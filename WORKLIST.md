...
|---|---|---|---|---|---|---|
| Power Leak — 3 parser bugs (dropped dynamic-X binding, wrongly-chained DealDamage+PreventDamage, wrong trigger-scope noun) plus a 4th, separate runtime bug found mid-fix (`TriggeringPlayer`-across-pause, see the follow-up row below); the CI regression that runtime fix caused (`swashbuckler_repro_3179_accept` reflexive-trigger event-amount collision) was found, fixed, and re-pushed as a 3rd commit — CI now fully green, PR open, awaiting maintainer review | old-school-1993-95 | in-progress | rykerwilliams-old-school | 2026-07-12 | `fix/power-leak-dynamic-prevention` | phase-rs/phase#5742 |
