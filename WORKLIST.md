...
|---|---|---|---|---|---|---|
| Power Leak — 3 parser bugs (dropped dynamic-X binding, wrongly-chained DealDamage+PreventDamage, wrong trigger-scope noun) plus a 4th, separate runtime bug found mid-fix (`TriggeringPlayer`-across-pause, see the follow-up row below) — PR open, **CI red**: the `PendingContinuation`/`drain_pending_continuation` engine fix regressed `swashbuckler_repro_3179_accept::accept_two_targets_grants_double_strike` (confirmed real regression, not flaky — passes clean on origin/main, fails on this branch); root-cause investigation in progress before fixing and re-pushing | old-school-1993-95 | in-progress | rykerwilliams-old-school | 2026-07-12 | `fix/power-leak-dynamic-prevention` | phase-rs/phase#5742 |
| Necromancy — plain Enchantment ETB-becomes-Aura reanimator (issue #640) — PR open, not yet merged. Corrected final root cause (differs from the earlier claim in this row): the ETB clause falls through the Animate Dead-class combinator (which only handles the quoted keyword-adoption clause) straight to `Effect::Unimplemented` — no target-bearing effect node ever built, zero target ever offered. `ReturnAsAura` independently confirmed the wrong building block regardless (non-targeted, self-return only; Necromancy needs a genuine CR 603.3d stack target). Fix: new whole-body "grant" recognizer + generalized shared chain builder (`EnchantGrantShape::Swap`/`GrantOnly`), 5 plan-review rounds, 1 impl-review round, all clean. 4 sibling cards (Last Voyage of the _____, Cloudform, Lightform, Rageform) share a similar template but use an unquoted restriction + different producer/sacrifice phrasing — explicitly deferred, correctly still `Unimplemented` | old-school-1993-95 | in-progress | rykerwilliams-old-school | 2026-07-13 | `fix/necromancy-etb-reanimation` | phase-rs/phase#5778 |
