...
**Current holder:** `void-mirror-603-4 since 2026-09-11T12:05Z (expected ~30m, lib rebuild for a review-requested test, shared warm target dir)`
...
|---|---|---|---|---|---|---|
| **[Parser + runtime bug, CR 603.4, issue #8807]** Void Mirror (MH2) — "Whenever a player casts a spell, if no colored mana was spent to cast it, counter that spell." The intervening-`if` never parses (`parse_no_mana_spent_clause` recognizes only the literal `"no mana was spent to cast "`), so the trigger degrades to an unconditional counter-every-spell. Second half: the runtime `ManaSpentCondition` evaluator substring-dispatches on `"no mana was spent"` and defaults everything else to `false`, so even a parsed clause would counter nothing. Fix targets the class (colored-vs-colorless payment gating) via the canonical `QuantityCheck { lhs: ManaSpentToCast { metric: DistinctColors }, EQ, Fixed(0) }` path rather than extending the legacy stringly-typed condition. | old-school-1993-95 | in-progress | void-mirror-603-4 | 2026-09-11 | fix/void-mirror-colored-mana-spent | phase-rs/phase#8809 (open) |
