...
**Current holder:** `36e2b430-7353-mutation since 2026-09-18T01:36:40Z (expected ~15m, #7353 mutation discrimination probe)`
...
| **[Per-opponent target slots, issue #7353]** Blatant Thievery — "For each opponent, gain control of target permanent that player controls" prompts for N targets and offers nothing selectable. MEASURED: the PARSER is exonerated — the existing unit test `effect_for_each_opponent_gain_control_uses_per_opponent_target_fanout` passes on upstream/main, lowering to `MultiTargetSpec::bounded(1, PlayerCount{Opponent})` with the filter bound to `ControllerRef::TargetPlayer`. This CONTRADICTS the issue's triage comment, which blamed an unbound filter. The gap is per-slot binding at selection time: `TargetSelectionConstraint`'s four variants are all CROSS-slot (DifferentTargetPlayers / DifferentObjectControllers / SameZoneOwner / TotalManaValue) and none binds slot i to opponent i; `MultiTargetSpec` is only {min,max}; the sole per-slot binder, `companion_target_player_legal_targets`, is gated on `trigger_source` + a damage-to-player event batch, which a SORCERY can never have, so it falls through to "offer every legal player". Class shared with #7419 Afterlife from the Loam and #7450 Vaevictis Asmadi. NOT yet reproduced at runtime — repro before design. | old-school-1993-95 | in-progress | 36e2b430 | 2026-09-18 | (tbd) | — |
## Done
