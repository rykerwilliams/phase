...
|---|---|---|---|---|---|---|
| Power Leak — 3 parser bugs + a 4th runtime bug found mid-fix (`TriggeringPlayer`-across-pause) + a CI regression that runtime fix caused (fixed) + a maintainer change-request (added a real production-pipeline integration test, `power_leak_dynamic_prevention.rs`, driving the actual parser + `stack::resolve_top` — closed and re-requested review 2026-07-14) — CI fully green, 4 commits, awaiting maintainer re-review | old-school-1993-95 | in-progress | rykerwilliams-old-school | 2026-07-12 | `fix/power-leak-dynamic-prevention` | phase-rs/phase#5742 |
