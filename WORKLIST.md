...
|---|---|---|---|---|---|---|
| **[CONFIRMED REAL ENGINE BUG]** Worldgorger Dragon self-loop ("Immortal Worldgorger Dragon" — Animate Dead/Necromancy reanimates WGD itself) is completely broken — WGD's LTB mass-return brings NOTHING back. Two earlier hypotheses (CR 303.4g graveyard-sweep on no-legal-target; keyword-reset-on-exile) were both empirically refuted this session via a hand-driven single-cycle repro. **Actual root cause**: WGD's ETB (`Effect::ChangeZoneAll`, mass-exile) publishes no tracked object set and no `exile_links` when it resolves; WGD's LTB (`Effect::ChangeZone{TrackedSet(0)}`) binds to nothing, since a chain-local `TrackedSet` can't bridge two separate resolutions (the ETB exile, then the LTB return after WGD is sacrificed). Even the alternative `ExiledBySource`/`TrackedBySource` link mechanism can't save it — `zones.rs:511-524` prunes that link when the source exits to the **graveyard** (only exits-to-exile preserve it), and WGD is sacrificed to the graveyard, not exiled, before its LTB fires. Needs real exile-link modeling (CR 610.3a) for the general "exile all / return the exiled cards" self-loop pattern (WGD-shaped; Mechtitan Core is architecturally the same shape). A clean `#[ignore]`d repro test exists in `crates/engine/src/game/casting_tests.rs` (`worldgorger_dragon_animate_dead_self_loop_single_cycle`, on branch `test/worldgorger-dragon-reanimation-combo`) asserting the CR-correct combo including untap + Piranha Marsh −1-life discriminators — fails precisely at the crux (plain land stuck in Exile instead of Battlefield). Necromancy hits the identical failure point (not yet in test code — add once fixed). **User approved proceeding to a real fix 2026-07-16** — planning next. | misparse-general | in-progress | rykerwilliams-old-school | 2026-07-16 | `test/worldgorger-dragon-reanimation-combo` | — |
