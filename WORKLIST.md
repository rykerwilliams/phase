...
|---|---|---|---|---|---|---|
| **[FIXED, PR OPEN, maintainer caught a real scope bug in review]** Worldgorger Dragon self-loop mass-return — root cause turned out simpler than first suspected: `trigger_is_etb_exile_pending_duration` (`parser/oracle.rs`) only matched singular `Effect::ChangeZone`→Exile, never mass `Effect::ChangeZoneAll`→Exile, so WGD's ETB never got the `Duration::UntilHostLeavesPlay` stamp that activates the engine's already-correct, already-tested `ExileLinkKind::UntilSourceLeaves`/`check_exile_returns` machinery (built for Journey to Nowhere/Oblivion Ring). Widened the match arm — one-line class-general fix, not a WGD special case. Two earlier hypotheses this session (CR 303.4g graveyard-sweep; keyword-reset-on-exile) were both empirically refuted before landing on this. **Maintainer (matthewevans) caught a real defect in the first pushed commit**: Realm Razer was initially claimed as a second confirmed fix (identical ETB shape), but its actual Oracle text is "return the exiled cards to the battlefield **tapped**" (`enter_tapped: true`) — the automatic exile-link return path performs an unmodified zone move and can't carry that rider, so the original widening would have silently returned Realm Razer's lands untapped, contradicting its printed text. **Fixed with a follow-up commit**: `trigger_is_ltb_return` now also requires the LTB return effect to have zero entry modifiers (tapped/attacking/transformed/counters/etc.) before pairing it with the mass-exile synthesis — Realm Razer is now correctly left unstamped (excluded) rather than silently dropping its modifier, until a modifier-carrying return mechanism exists. WGD itself has no such modifiers and remains fixed. **Confirmed remaining gap, now fixed in a follow-up PR**: the returning Animate Dead/Necromancy Aura itself still couldn't re-attach to WGD-in-graveyard — `legal_aura_attachment_targets` (`zone_pipeline.rs`) only scanned the battlefield for legal Aura hosts, never graveyards. Fixed in phase-rs/phase#6072 (see row below); the WGD self-loop test on this branch still needs reconciling with that fix once both PRs are settled. | misparse-general | in-progress | rykerwilliams-old-school | 2026-07-16 | `test/worldgorger-dragon-reanimation-combo` | phase-rs/phase#6055 |
| **[FIXED, PR OPEN]** Aura-attachment host search (`legal_aura_attachment_targets`, `game/zone_pipeline.rs`) only scanned the battlefield for legal hosts when a non-cast Aura entry needs one chosen (CR 303.4f/g) — never graveyards/hand. Rewritten to resolve the search zone(s) from the Aura's own `enchant_filter.extract_zones()` (defaulting to battlefield when unconstrained — zero change for ordinary Auras), reusing the existing `object_count_matching_ids` pattern (`game/quantity.rs`); uses `matches_target_filter` not the spell-targeting enumerator (CR 115.1b: this is a choice, not a target, so hexproof/shroud must never filter it). Confirmed class-general: fixes Animate Dead, Dance of the Dead, Necromancy, Spellweaver Volute (graveyard-scoped) and Don't Worry About It (hand-scoped). Closes the final leg of the Worldgorger Dragon self-loop combo (#6055 fixed the mass-return mechanism; this fixes the Aura's own re-attachment) — reasoned through, not yet reconciled with that test on the other branch/PR. 1 plan round (CLEAN), 1 impl round (CLEAN, independent live-revert check done twice). | misparse-general | in-progress | rykerwilliams-old-school | 2026-07-17 | `fix/aura-attachment-graveyard-host-search` | phase-rs/phase#6072 |
