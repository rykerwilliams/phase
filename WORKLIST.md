...
|---|---|---|---|---|---|---|
| **[FIXED, PR OPEN]** Worldgorger Dragon self-loop mass-return — root cause turned out simpler than first suspected: `trigger_is_etb_exile_pending_duration` (`parser/oracle.rs`) only matched singular `Effect::ChangeZone`→Exile, never mass `Effect::ChangeZoneAll`→Exile, so WGD's ETB (and Realm Razer's, second confirmed real-card match) never got the `Duration::UntilHostLeavesPlay` stamp that activates the engine's already-correct, already-tested `ExileLinkKind::UntilSourceLeaves`/`check_exile_returns` machinery (built for Journey to Nowhere/Oblivion Ring). Widened the match arm — one-line class-general fix, not a WGD special case; blast radius bounded (only fires when a matching LTB-return trigger also exists on the card). Two earlier hypotheses this session (CR 303.4g graveyard-sweep; keyword-reset-on-exile) were both empirically refuted before landing on this. **Confirmed remaining gap, honestly scoped out**: the returning Animate Dead/Necromancy Aura itself still can't re-attach to WGD-in-graveyard — `legal_aura_attachment_targets` (`zone_pipeline.rs`) only scans the battlefield for legal Aura hosts, never graveyards, so a graveyard-targeting `Enchant` ability (Animate Dead/Necromancy/Dance of the Dead class) can never find a legal host on non-cast re-entry even when one legally exists. This is a separate, wider fix (extending the CR 303.4f/g host-search scope) — tracked as its own follow-up below, not yet started. PR: 1 plan round (root cause verified, CLEAN), 1 impl round (found + fixed a stray tracked-file regen corruption + a stale doc comment during review), CI pending. | misparse-general | in-progress | rykerwilliams-old-school | 2026-07-16 | `test/worldgorger-dragon-reanimation-combo` | phase-rs/phase#6055 |
