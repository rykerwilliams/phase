...
|---|---|---|---|---|---|---|
| Worldgorger Dragon + Animate Dead/Necromancy — the famous notorious combo. Plan round 1's "Variant B" bug hypothesis (Aura wrongly swept to graveyard instead of staying in exile on WGD's LTB mass-return, for a NON-self-targeted reanimation) was refuted by adversarial review, then user domain-expert pushback surfaced a real dispute: found an official WotC ruling (WGD's card-data.json rulings, dated 2022-12-08) confirming an exile-stranded Aura CANNOT reattach to a permanent entering simultaneously and stays exiled with no other legal target — but whether this general ruling (written for normal battlefield-permanent-enchanting Auras) cleanly transfers to Animate Dead's unusual "enchant creature CARD IN A GRAVEYARD" ability is unconfirmed; **user is checking with a judge friend, Variant B's expected-outcome assertion is ON HOLD pending that**. **Reprioritized to "Variant A" — the real self-targeting "Immortal Worldgorger Dragon" loop** (Animate Dead/Necromancy reanimates WGD itself; WGD's ETB exiles the Aura; Aura's LTB-sacrifice fires, sacrificing WGD to the graveyard as a genuinely separate prior event; WGD's own LTB returns the Aura from exile; Aura re-attaches to WGD-in-graveyard per CR 303.4f since it's a pre-existing target, not simultaneous — no dispute here since there's no simultaneous-entry ambiguity), single-cycle only (no loop-detection infra, out of scope). Also verifying the combo's actual real-world payoff: a tapped land among the exiled permanents should return UNTAPPED each cycle (fresh object, no "enters tapped" ability) — the "infinite mana" engine this combo is actually played for. Necromancy's `card-data.json` entry is stale relative to its own PR #5778 fix (`GenericEffect`-rooted trigger) — test must construct via live `parse_oracle_text` (per the existing `cast_necromancy_and_fire_etb`/`reanimate_grizzly_via_animate_dead` precedents in `casting_tests.rs`), not trust the JSON. | misparse-general | in-progress | rykerwilliams-old-school | 2026-07-16 | `test/worldgorger-dragon-reanimation-combo` | — |
