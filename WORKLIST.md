...
|---|---|---|---|---|---|---|
| Worldgorger Dragon + Animate Dead/Necromancy — the famous notorious combo (ETB exiles all your other permanents incl. the attached reanimating Aura → Aura's own LTB delayed-sacrifice fires → Dragon sacrificed → Dragon's own LTB mass-returns everything). Individually-correct primitives (`ChangeZoneAll`/`TrackedSet`, the reanimation Aura's destination-agnostic delayed trigger) architecturally look right, but **zero runtime test exists anywhere** for this interaction — discovered while scanning a decklist that plays WGD with both reanimation Auras. Scope widened to cover both Animate Dead AND Necromancy (merged separately as #5778, reuses the same Aura-attach-reanimation chain-builder architecture — same class of risk). Needs a discriminating integration test before trusting it; may reveal a real gap once actually driven end-to-end (same lesson as the Animate Dead fix itself: individually-correct pieces can still fail composed). | misparse-general | in-progress | rykerwilliams-old-school | 2026-07-16 | `test/worldgorger-dragon-reanimation-combo` | — |
