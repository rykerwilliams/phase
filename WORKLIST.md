...
**Current holder:** `custom-format-2a since 2026-09-09T16:01:02Z (expected ~10m, warm target: PR 8765 review fix)`
...
| **[Engine+parser bug, issue #5653]** Chains of Mephistopheles / Magus of the Chains — the two result-referential riders ("If the player discards a card this way, they draw a card. If the player doesn't discard a card this way, they mill a card.") parse with condition=None, so all three actions run unconditionally: a player with cards in hand discards AND draws AND mills. Correct: discard, then EITHER draw OR mill. Research so far (worktree `chains-5653`, based on upstream/main fe947b898): the runtime plumbing ALREADY EXISTS — `SpellContext.direct_discard_result: Option<DiscardedCardResult>` is stamped at the Discard->direct-child hand-off (`effects/mod.rs:13750`) and cleared for any non-Discard parent (`:3111-3118`), and `AbilityCondition::DiscardedCardMatchesFilter { filter }` already reads it for the Recruit class. So this looks like a PARSER gap, not a missing engine primitive: "discards a card this way" should reach `DiscardedCardMatchesFilter { filter: Any }` and the negative polarity should compose with the existing `AbilityCondition::Not`. Open question being verified: whether the else/second-rider node is a DIRECT child of the Discard node (stamped) or a grandchild (cleared) — that determines whether the two riders can be sub+else under the Discard or need a different shape. | old-school-1993-95 | in-progress | chains-5653 | 2026-09-09 | fix/result-referential-discard-conditions | — |
