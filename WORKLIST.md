...
| Necromancy — plain Enchantment ETB-becomes-Aura reanimator (issue #640); confirmed root cause is the ETB clause parsing via the wrong combinator (Old-Growth-Troll-class `ReturnAsAura` self-transform), silently dropping the targeted graveyard-reanimation sentence entirely — zero target ever offered. `ReturnAsAura` itself confirmed the wrong building block (non-targeted, self-return only). Also found 4 sibling cards sharing the same "becomes an Aura with X, [producer effect], attach self to result" template (Last Voyage of the _____, Cloudform, Lightform, Rageform) — scoping this PR to Necromancy, designing for the class | old-school-1993-95 | in-progress | rykerwilliams-old-school | 2026-07-13 | `fix/necromancy-etb-reanimation` | — |
