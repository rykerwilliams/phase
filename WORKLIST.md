...
| **[Follow-up, in-progress]** Reconcile the Worldgorger Dragon self-loop test (`worldgorger_dragon_animate_dead_self_loop_single_cycle`, on `test/worldgorger-dragon-reanimation-combo`/#6055) with #6072 (merged) — its assertions currently document the Aura staying stuck in exile as a "known gap"; now that #6072 is merged, that gap is fixed and the test's final assertions need to flip to the actual closed loop (Aura re-attaches to WGD-in-graveyard, reanimates it). Working on this now. | misparse-general | in-progress | rykerwilliams-old-school | 2026-07-17 | `test/worldgorger-dragon-reanimation-combo` | phase-rs/phase#6055 |
