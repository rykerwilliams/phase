...
|---|---|---|---|---|---|---|
| Power Leak — 3 parser bugs + a 4th runtime bug found mid-fix (`TriggeringPlayer`-across-pause) + a CI regression that runtime fix caused (fixed as a 3rd commit) — CI fully green, but maintainer (@matthewevans) requested changes 2026-07-14: existing `TriggeringPlayer` runtime tests use a hand-built `ResolvedAbility` chain + manual trigger-context clearing, bypassing the real parser and `stack::resolve_top` — the two boundaries this PR actually touches. Needs a genuine integration test driving Power Leak's real parsed Oracle text through an actual upkeep trigger, asserting the enchanted controller (not the Aura's) takes exactly `2-X` damage. In progress. | old-school-1993-95 | in-progress | rykerwilliams-old-school | 2026-07-12 | `fix/power-leak-dynamic-prevention` | phase-rs/phase#5742 |
