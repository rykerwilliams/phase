...
**Current holder:** `leng-7510 since 2026-09-09T19:23Z (expected ~40m, cold worktree, integration target)`
...
| **[Engine+parser bug, issue #8388]** Dragon Whelp — "If this ability has been activated four or more times this turn, sacrifice this creature at the beginning of the next end step" reached no condition parser, so the sacrifice rider ran UNCONDITIONALLY (sacrificed after one activation). Invisible to coverage: `line_has_condition_text` (`coverage.rs`) carried "this ability has been activated" on its structural-exemption list, so the card read as supported while the clause was dropped. Fixed by PARAMETERIZING `AbilityCondition::NthResolutionThisTurn { n }` into `AbilityUseCountThisTurn { tally, comparator, n }` rather than adding a sibling — both axes are demanded by printed cards (tally: Resolved 27 / Activated 4; comparator: EQ 26 / GE 4, plus Rose Room Treasurer's "first or second time" which the EQ-only shape could never express). The `Activated` tally reads the CR 602.2a ledger `activated_abilities_this_turn`, already keyed by the same `(source_id, ability_index)` the ability carries, so no new plumbing. Serde back-compat via `#[serde(alias)]` + elided defaults. Class: Dragon Whelp, Nalathni Dragon, Farrelite Priest, Initiates of the Ebon Hand. NOTE for anyone nearby: two comments claiming activated abilities lack an `ability_index` stamp were STALE and are corrected — both stack paths do stamp it. | old-school-1993-95 | in-progress | dragon-whelp-8388 | 2026-09-09 | fix/ability-use-count-this-turn | phase-rs/phase#8764 |
