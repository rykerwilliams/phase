...
|---|---|---|---|---|---|---|
| Worldgorger Dragon + Animate Dead/Necromancy — the famous notorious combo. Plan round 1 hypothesized a real CR 303.4g bug (Aura wrongly swept to graveyard instead of staying in exile on WGD's LTB mass-return); adversarial review REFUTED this — traced the actual code path WGD's LTB uses (`ChangeZone{TrackedSet}` → `execute_zone_move_with_applied`'s `[] => Done` branch, `zone_pipeline.rs`) and confirmed it already does the CR-correct thing (Aura stays in exile). The plan had mis-traced 2 unrelated code paths (`return_as_aura.rs`/the `BecomeCopy` consult in the same file) that only apply to auras becoming Auras while already on the battlefield, not the exile-mass-return case. **Reclassified: test-only, same pattern as Land's Edge (#5547)** — existing code is believed correct, needs a discriminating regression-pin test (expect: Aura stays in `Zone::Exile`, reanimated creature returns unattached at base P/T, not spuriously sacrificed). One residual empirical question the reviewer flagged: whether the exiled Aura retains its printed keywords/subtypes at the moment the branch runs — if the test unexpectedly shows graveyard instead of exile, that's a DIFFERENT bug (keyword-reset-on-exile), not the one originally hypothesized. Necromancy's `card-data.json` entry is stale relative to its own PR #5778 fix (`GenericEffect`-rooted trigger) — test must construct via live `parse_oracle_text` (per the existing `cast_necromancy_and_fire_etb`/`reanimate_grizzly_via_animate_dead` precedents in `casting_tests.rs`), not trust the JSON. | misparse-general | in-progress | rykerwilliams-old-school | 2026-07-16 | `test/worldgorger-dragon-reanimation-combo` | — |
