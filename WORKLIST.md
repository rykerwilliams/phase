...
|---|---|---|---|---|---|---|
| **[FIXED, PR OPEN, fully closes end-to-end]** Worldgorger Dragon self-loop combo — the "famous notorious combo" the worklist named at the top of this whole investigation. Root cause chain (3 distinct bugs found across this session, all now fixed): (1) `trigger_is_etb_exile_pending_duration` only matched singular `ChangeZone`→Exile, never mass `ChangeZoneAll`→Exile, so WGD's ETB never got the `Duration::UntilHostLeavesPlay` stamp — fixed by widening the match, a one-line class-general fix (also fixes Realm Razer's ETB detection, though Realm Razer itself stays correctly excluded — see (2)). Two earlier hypotheses (CR 303.4g graveyard-sweep; keyword-reset-on-exile) were empirically refuted before landing on this. (2) Maintainer (matthewevans) caught a real defect: Realm Razer's LTB has an `enter_tapped: true` rider the automatic exile-link return can't carry — fixed by requiring the LTB return effect to have zero entry modifiers before pairing it with the mass-exile synthesis; Realm Razer is now correctly excluded rather than silently dropping its modifier. **(2b) Follow-up caught across 4 more review rounds**: excluding Realm Razer left it showing as falsely "fully supported" in coverage/semantic-audit (no visible gap, even though it has no working return at all) — fixed by parameterizing `DocumentRelationIr::EtbExileLtbReturn` with an `outcome` (`DurationStamped`/`ModifierUnsupported{fragment}`) and attaching `Effect::unimplemented("modifier_bearing_linked_return", ...)` to the LTB trigger when the automatic return can't carry its modifier, so `card_face_gaps`/coverage correctly flags it — plus a coverage-level regression test (not just a duration-is-None check, per the maintainer's explicit ask). Structurally guaranteed not to false-flag a genuinely-supported card (only reachable for a return the parser captured an actual entry modifier on). (3) The returning Animate Dead/Necromancy Aura itself couldn't re-attach to WGD-in-graveyard — `legal_aura_attachment_targets` only scanned the battlefield, never graveyards — fixed and merged separately as phase-rs/phase#6072, then this branch merged that fix and updated `worldgorger_dragon_animate_dead_self_loop_single_cycle` to prove the FULL single-cycle loop closes end-to-end: Aura re-attaches, its ETB re-fires, WGD is reanimated a second time. Also fixed a pre-existing CR 610.3a→610.3 citation error caught along the way. Awaiting maintainer's final re-review on CI-green code. | misparse-general | in-progress | rykerwilliams-old-school | 2026-07-16 | `test/worldgorger-dragon-reanimation-combo` | phase-rs/phase#6055 |
