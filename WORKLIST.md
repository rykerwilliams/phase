...
**Current holder:** `void-mirror-603-4 since 2026-09-15T14:17Z (expected ~30m, Citadel of Pain repro test then fix, shared warm target dir)`
...
|---|---|---|---|---|---|---|
| **[Trigger quantity binding, CR 603.2 + CR 109.5, issue #6508]** Citadel of Pain — "At the beginning of each player's end step, this enchantment deals X damage to that player, where X is the number of untapped lands they control." Reported: only the controller takes damage. Triage says the where-X count binds "they" to the controller instead of the end-step player. No commit or test cites it; the sibling `ScopedPlayer` binding is pinned only for "each player …" effects (Acidic Soil, Biorhythm), not phase-scoped triggers. Reproducing at runtime with different untapped-land counts per player to separate 'trigger only on controller's step' from 'count uses controller's lands'. | old-school-1993-95 | in-progress | void-mirror-603-4 | 2026-09-15 | fix/citadel-of-pain-6508 | — |
