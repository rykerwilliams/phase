...
|---|---|---|---|---|---|---|
| Nether Spirit — dropped intervening-if ("only creature card in your graveyard" self-reanimation trigger); confirmed via `card-data.json` (`condition: null`), part of open cluster issue #1384 (no dedicated child issue yet — existing children #2861/#2867/#2868 are closed, cover different cards). New `parse_source_is_only_type_in_zone` combinator (composes existing building blocks only), zero runtime changes needed (existing `trigger_condition_source_zones`/`stamp_self_return_origin_from_trigger_condition` auto-derive `trigger_zones`/`ChangeZone.origin` from the `And{SourceInZone,...}` shape — same path as the Jocasta, Automaton Avenger precedent). Plan reviewed (1 round, citation/line-number corrections only, architecture validated sound); implementation independently reviewed CLEAN (1 round). 3 parser unit tests + 1 trigger-shape test + 6 integration tests, all green; clippy clean; live-revert proof done. PR opened. | misparse-general | in-progress | rykerwilliams-old-school | 2026-07-14 | `fix/nether-spirit-self-reanimation-condition` | phase-rs/phase#5884 |
