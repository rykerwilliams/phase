...
|---|---|---|---|---|---|---|
| Power Leak — 3 parser bugs + a 4th runtime bug found mid-fix (`TriggeringPlayer`-across-pause) + a CI regression that runtime fix caused (fixed, Swashbuckler) + a maintainer change-request (added `power_leak_dynamic_prevention.rs`, closed and re-requested review 2026-07-14) — **2nd CI regression found 2026-07-15**: after re-requesting review, `Rust tests (shard 1/2)` + `(shard 2/2)` both now fail — 5 failures, all in roll-die/d20 subability quantity resolution (`roll_die_multi_count_subability_reads_total_results`, `roll_die_subability_reads_roll_not_trigger_event`, `ancient_copper_dragon_roll_d20`, `resolve_event_context_amount_prefers_die_result_over_event`, `reflexive_entry_lifts_carried_die_result_into_resolution_scope`) — all read a stale surviving combat-damage amount instead of the actual die/d20 result. Same failure class as the 1st (Swashbuckler) regression: the `with_reflexive_resolution_scope`/`SUPPRESS_ENCLOSING_TRIGGER_EVENT_AMOUNT` fix from that round wraps the *entire* `try_begin_reflexive_target_selection_inner` body, so it now also suppresses a roll-die/d20 subability's own legitimate enclosing-die-result lookup (not just the target-count leak it was built to fix), falling through to a wrong `last_effect_count` tier. Needs narrowing: the suppression must distinguish "enclosing target-count" (wrong to leak) from "enclosing die/d20 result" (legitimate, must pass through) rather than blocking the whole cascade tier uniformly. **Fixed 2026-07-15** (`c0fba2f15`, matthewevans, co-authored with rykerwilliams-old-school): split the merged `enclosing_trigger_context_amount` back into `enclosing_trigger_match_count` + `enclosing_trigger_event_amount`, interleaving `die_result_this_resolution` between them so precedence is match-count → die-result → event-amount; both tiers stay suppressible via the same `with_reflexive_resolution_scope` flag. Verified: all 5 previously-failing tests pass, full engine lib + integration suites green locally, and PR CI is now all-green (`Rust tests` shard 1/2 + 2/2, `Rust lint`, `Rust (fmt, clippy, test, coverage-gate)`, WASM/Tauri/frontend/card-data all SUCCESS). Note: an independent equivalent fix was drafted concurrently in worktree `power-leak-ci-fix2` but discarded unpushed once the functionally-identical upstream commit was found (multi-agent-safety — no competing commit pushed). PR is CI-green, awaiting final review/merge. | old-school-1993-95 | in-progress | rykerwilliams-old-school | 2026-07-12 | `fix/power-leak-dynamic-prevention` | phase-rs/phase#5742 |
| Nether Spirit — dropped intervening-if ("only creature card in your graveyard" self-reanimation trigger); confirmed via `card-data.json` (`condition: null`), part of open cluster issue #1384 (no dedicated child issue yet — existing children #2861/#2867/#2868 are closed, cover different cards) | misparse-general | in-progress | rykerwilliams-old-school | 2026-07-14 | `fix/nether-spirit-self-reanimation-condition` | — |
