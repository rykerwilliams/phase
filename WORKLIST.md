...
|---|---|---|---|---|---|---|
| Power Leak — 3 parser bugs (dropped dynamic-X binding, wrongly-chained DealDamage+PreventDamage, wrong trigger-scope noun) plus a 4th, separate runtime bug found mid-fix (`TriggeringPlayer`-across-pause, see the follow-up row below) — PR open. **CI regression fixed and re-pushed**: the `PendingContinuation`/`drain_pending_continuation` fix had regressed `swashbuckler_repro_3179_accept::accept_two_targets_grants_double_strike` (a CR 603.12 reflexive trigger's own "that many" losing to the restored enclosing trigger's event-amount); fixed via a thread-local suppression scope covering exactly `try_begin_reflexive_target_selection`'s dynamic extent, 2 plan-review rounds + 2 impl-review rounds, all clean, full lib+integration suites green, pushed as a 3rd commit | old-school-1993-95 | in-progress | rykerwilliams-old-school | 2026-07-12 | `fix/power-leak-dynamic-prevention` | phase-rs/phase#5742 |
