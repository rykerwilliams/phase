...
//!   listed zones; command-zone objects use the emblem-or-opt-in gate). Every
//!   statics gather delegates to it — `active_static_definitions` (which also
//!   layers CR 113.6g's stack exception for self-referential
...
//!   the two `layers.rs` continuous / combat-assignment gathers, and
//!   `combat::compute_combat_tax` — so they cannot disagree on zone-of-function.
//!   Callers therefore never need to pre-filter statics by zone.
